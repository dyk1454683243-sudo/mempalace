...
- **`mempalace window` walks a wing chronologically; `mempalace source` lists one file's drawers in order** (`HEAD` — pending resolution)
  ...
  insertion-ordered with no time filter at all, so against a
  201K-drawer wing a date is ~200 pages away. And search hits carry a
  ``source_file``, whose obvious follow-up — "the drawers from *that*
  file, in order" — had no command either.
  ...
  **The time semantics are the palace's existing ones, not new ones.**
  ...
  surfaces it, so drift is structurally prevented). What changed is the
  performance envelope: the filter is now a SQL predicate instead of a
  Python pass after fetching every row.
  ...
  no index on any time value; #500's "pgvector backend already indexes
  it" describes upstream's table, not the fork's. Time lives in
  ``metadata->>'filed_at'``. And **``filed_at`` carries two timezone
  conventions** (915,562 naive host-local, 5,474 ``Z``/UTC), so a
  wall-clock walk places 0.6% of drawers up to 7h out of position. That is
  a cross-surface defect rather than this verb's, filed as #506 with the
  ...
  **not** the version (``_get_daemon_rest`` collapses 404/401/403 into
  ``None``, so reusing it would report a key mismatch as a version
  problem — a refusal naming the wrong reason); 400/422 → 64 carrying the
  daemon's own message; transport/5xx → 2; an empty window → 1.

  Verified end to end against a real daemon on a scratch postgres, and
  against the **production** daemon for the 404 path (which returns 404
  for ``/window`` today, so the message was tested against the condition
  it describes rather than a simulation). Chunk ordering came back 0, 1,
  10 — numeric, not lexical, which is the "in order" #502 asks for.

  *Tests:* 35 (test_cli_window_source — three-layer wiring with the probes verified to fail independently, every exit-code row, the auth-vs-version distinction in both directions, and a real-binary probe)
