...
class TestTheMessageIsWrittenOnce:
    def test_only_the_two_helpers_still_carry_the_literal(self):
        ...
        `_fail_daemon` is the one that stays. `_daemon_tool_or_fail` keeps its own
        deliberately: it exits 1 for a transport failure and 2 for a JSON-RPC error
        from a daemon that answered, a distinction `_fail_daemon` does not yet make.
        Folding it in would have destroyed that — it converges in the exit-code PR,
